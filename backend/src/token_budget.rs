...
impl TokenUsageMonitor {
    ...
    pub fn optimize_attitude_context(&mut self, attitudes: Vec<CompanionAttitude>) -> Vec<CompanionAttitude> {
    ...
    fn calculate_attitude_significance(attitude: &CompanionAttitude) -> f32 {
        let dimensions = [
            ...
            attitude.curiosity, attitude.respect, attitude.suspicion, attitude.gratitude,
            attitude.jealousy, attitude.empathy
        ];
    ...
    fn get_top_dimensions(&self, attitude: &CompanionAttitude, n: usize) -> Vec<String> {
        ...
        dimensions.sort_by(|a, b| b.1.abs().partial_cmp(&a.1.abs()).unwrap_or(std::cmp::Ordering::Equal));
    ...
    pub fn optimize_third_party_context(&mut self, third_parties: Vec<ThirdPartyIndividual>) -> Vec<ThirdPartyIndividual> {
    ...
    pub fn get_usage_statistics(&mut self) -> TokenUsageStatistics {
        ...
        let remaining_response_tokens = self.budget.response_buffer.min(
            self.budget.total.saturating_sub(self.current_usage.total_context_tokens)
        );
        
        let total_utilization = self.current_usage.total_context_tokens + remaining_response_tokens;
        let utilization_percentage = (total_utilization as f32 / self.budget.total as f32 * 100.0) as u8;
        ...
        TokenUsageStatistics {
            ...
            overflow_risk: self.current_usage.total_context_tokens > (self.budget.total as f32 * 0.85) as usize,
        }
    }
    ...
    pub fn get_optimization_suggestions(&self) -> Vec<String> {
        ...
        let utilization = (self.current_usage.total_context_tokens as f32 / self.budget.total as f32 * 100.0) as u8;
        if utilization > 90 {
            suggestions.push("Context utilization above 90% - consider optimizing content".to_string());
        } else if utilization < 50 {
            suggestions.push("Low context utilization - could include more conversation history".to_string());
        }
...
impl TokenUsageStatistics {
    pub fn print_detailed_stats(&self) {
        ...
        println!("   System Prompt: {}/{} tokens", self.current_usage.system_tokens, self.budget.system_prompt);
        println!("   Attitudes: {}/{} tokens", self.current_usage.attitude_tokens, self.budget.attitude_data);
        println!("   Third-party Info: {}/{} tokens", self.current_usage.third_party_tokens, self.budget.third_party_info);
        println!("   Messages: {}/{} tokens", self.current_usage.message_tokens, self.budget.recent_messages);
        println!("   Response Buffer: {} tokens available", self.remaining_response_tokens);
        println!("   Total Context: {} tokens", self.current_usage.total_context_tokens);
        ...
        println!("   Messages compressed: {}", self.optimization_stats.messages_compressed);
        println!("   Attitudes filtered: {}", self.optimization_stats.attitudes_filtered);
        println!("   Third-parties filtered: {}", self.optimization_stats.third_parties_filtered);
        println!("   Overflow events: {}", self.optimization_stats.overflow_events);
        
        if self.optimization_stats.total_savings > 0 {
            println!("   Total tokens saved: {}", self.optimization_stats.total_savings);
        }
    }
}
...
mod tests {
    ...
    fn test_token_budget_allocation() {
        ...
        let total_allocated = budget.system_prompt + budget.attitude_data + 
                            budget.third_party_info + budget.recent_messages + 
    ...
    fn test_attitude_significance_calculation() {
        ...
        let high_score = TokenUsageMonitor::calculate_attitude_significance(&high_significance_attitude);
        let low_score = TokenUsageMonitor::calculate_attitude_significance(&low_significance_attitude);
    ...
    fn test_overflow_detection() {
        ...
        let suggestions = monitor.get_optimization_suggestions();
        assert!(suggestions.iter().any(|s| s.contains("overflow") || s.contains("90%")));
    }
