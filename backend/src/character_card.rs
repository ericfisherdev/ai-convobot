use base64::{alphabet::STANDARD, engine::GeneralPurpose, engine::GeneralPurposeConfig, Engine};
...
impl CharacterCard {
    pub fn load_character_card(bytes: &[u8]) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let character_base64_option: Option<String> = reader
            .info()
        ...
        let character_base64: String = match character_base64_option {
            Some(v) => v,
            None => {
                let text_chunk_start = bytes
                    .windows(9)
                    .position(|window| window == b"tEXtchara")
                    .ok_or_else(|| {
                    Box::<dyn std::error::Error>::from("No tEXt chunk with name 'chara' found")
                })?;
                let text_chunk_end = bytes
                    .windows(4)
                    .rposition(|window| window == b"IEND")
                    .ok_or_else(|| {
                        Box::<dyn std::error::Error>::from("No tEXt chunk with name 'chara' found")
                    })?;
                String::from_utf8_lossy(&bytes[text_chunk_start + 10..text_chunk_end - 8])
                    .to_string()
            }
        };
