...
fn fresh_database_has_default_compaction_config_and_persists_updates() {
    ...
    // Defaults to `false`: #179's `/api/compaction/{id}/commit` route is the
    // production-reachable path to `compaction::production_commit_deps` /
    // `commit::commit`, so `compaction::persons::PersonsObserver` (#177) is
    // now the trusted source of third-party people and the pre-compaction
    // heuristic gate (`chat_turn::preprocess_user_message`) defaults off.
